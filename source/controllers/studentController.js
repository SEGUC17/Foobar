...
const StudentController = {
  ...
  approveReservation(req, res) {
    ...
    addReview(req, res) { //used to add a review with a rating to the service providers
    ...
    addComment(req, res) { //adding a comment to a review
    ...
    seeProgress(req, res) { //will be used in sprint 3 to implement a graph that shows the progress of the student
      const token = req.headers['jwt-token'];
      jwt.verify(token, (decoded) => {
        if (decoded.type === 2) {
          Skill.find({
    ...
    applyOffer(req, res) { //applying for an offer
      const token = req.headers['jwt-token'];
      jwt.verify(token, (decoded) => {
        if (decoded.type === 2) {
          Offer.findOne({
    ...
    viewStudent(req, res) { //viewing the student's profile
    ...
    // viewing the reviews on the sp's page
    viewReviews(req, res) {
      const token = req.headers['jwt-token'];
      jwt.verify(token, (decoded) => {
        if (decoded.type === 2) {
          ...
          Review.find({
            sp_id: req.body.sp_id
          }).populate('reviewer_id').populate('sp_id').exec((
            err,
            reviews) => {
            if (err) {
              res.status(500).json({
    ...
    editPassword(req, res) { //changing ur password
        ...
        if (decoded.type === 2) {
          User.find({
            ...
            if (err)
              ...
              req.checkBody('oldPassword',
                'Passwords do not match').equals(user[0].password);
              req.checkBody('confirmNewPassword',
                'Confirm Password does not match newPassword'
    ...
    editStudent(req, res) { //editing the student's profile
