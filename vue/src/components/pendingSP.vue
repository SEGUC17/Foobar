...
methods:{
    getAllPendingSP: function () {
      this.$http.get('http://54.77.11.251:3000/api/admins/pendingSPRequests/',{headers : { 'jwt-token' : localStorage.getItem('id_token')}}).then(response => {
    ...
    approve: function(pending){
      ...
      // if(x){
        this.$http.post('http://54.77.11.251:3000/api/admins/pendingSPRequests',{"id":pending._id,"approve":true,"name":pending.name,"email":pending.email,"phone_number":pending.phone_number,"description":pending.description},{headers : { 'jwt-token' : localStorage.getItem('id_token')}} ).then(response => {
          swal(
            'Success!',
