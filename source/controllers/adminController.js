...
const adminController = {
  ...
  //a method used to block a student
  blockS(req, res) {
    ...
    //a method to block a service provider from the system
    blockSP(req, res) {
      const token = req.headers['jwt-token'];
      jwt.verify(token, (decoded) => {
        if (decoded.type === 1) {
    ...
    //a method to delete the student from the system ( deleted student interactions will be deleted like reviews)
    deleteS(req, res) {
      const token = req.headers['jwt-token'];
      jwt.verify(token, (decoded) => {
        if (decoded.type === 1) {
    ...
    getAllStudents(req, res) { // viewing all students
      const token = req.headers['jwt-token'];
      jwt.verify(token, (decoded) => {
        if (decoded.type === 1) {
    ...
    // used to find out which interests are being chosen the most and least
    reviewDataAnalysis(req, res) {
      ...
      const token = req.headers['jwt-token']; + jwt.verify(token, (decoded) => {
        ...
        if (decoded.type === 1) {
          StudentInterest.find([]).populate('interest_id').exec((err,
            interests) => {
    ...
    // to add another admin to the system
    addAdmin(req, res) {
      const token = req.headers['jwt-token'];
      jwt.verify(token, (decoded) => {
        if (decoded.type === 1) {
