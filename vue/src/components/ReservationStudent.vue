...
methods:{
    getReservations: function () {
      this.$http.get('http://54.77.11.251:3000/api/students/reservations/view', {headers : {'jwt-token' : localStorage.getItem('id_token')}}).then(response => {
        this.reservations=response.data.data.reservations
      })
    }, refund: function(id, charge){
       if(localStorage.getItem('id_token')!=null){
          this.$http.post('http://54.77.11.251:3000/api/refund',{"charge_id":charge},{headers : {'jwt-token' : localStorage.getItem('id_token')}}).then(response=> {
            this.$http.post('http://54.77.11.251:3000/api/changeStatus',{"id":id},{headers : {'jwt-token' : localStorage.getItem('id_token')}}).then(response=> {
              this.getReservations();
            })
          })
