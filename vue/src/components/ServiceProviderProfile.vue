...
<template>
   ...
   <div>
		...
		<div class="tab_container">
          ...
          <h3>{{user.name}}</h3>
          <center><img class="img-circle img-responsive" v-if="user && user.profileimg.path" :src="'http://52.210.115.35:3000/'+user.profileimg.path.replace('public','')" style="height:150px; width:150px">
...
<div id="makan2" style="width:80%">
 ...
 methods:{
    getServiceProvider: function (param) {
        if(param!=null){
        console.log(param)
      let route ='http://52.210.115.35:3000/api/students/sP/'.concat(param);
